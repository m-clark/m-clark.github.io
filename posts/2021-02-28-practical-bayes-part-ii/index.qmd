...
draft: false
tags: [bayesian, stan, <span class="pack" style = "">rstanarm</span>, brms, errors, warnings, posterior predictive checks, model averaging, loo, waic]
categories:
  - bayesian
...
execute: 
  cache: true
share:
    permalink: 'https://m-clark.github.io/posts/2021-02-28-practical-bayes-part-ii/'
    description: 'Practical Bayes Part II'
...
Thankfully, the [brms]{.pack style=""} package makes this very easy to do. We will check the following types of priors, that range from default settings to increasing specification for all parameters of interest.
...
We can use [pp_check]{.func style=""} to examine the prior-generated data versus the observed target `y`, but I wait to show them all together at the end. Note the argument `sample_prior`, which we set to `'only'`.
...
{
  {
    pp_check(model_default_prior, ndraws = 50, alpha = .2) +
      labs(title = 'Defaults')
  }/
    # theme(axis.text.x = element_text(size= 6)) +
    {
      pp_check(model_0_norm_b_0_10, ndraws = 50, alpha = .2) +
        labs(title = 'Normal b (diffuse)')
    }/
    {
      pp_check(model_auto_prior, ndraws = 50, alpha = .2) +
...
{
  {
    pp_check(model_0_norm_b_0_1, ndraws = 50, alpha = .2) +
    labs(title = 'Normal b tighter')
  } /
{
  pp_check(model_0_norm_b_0_1_norm_Int, ndraws = 50, alpha = .2) +
    labs(title = 'Add Int prior') 
  } /
    pp_check(model_0_norm_b_0_1_norm_Int_sigma, ndraws = 50, alpha = .2) +
...
So given that our target variable is between `r round(min(main_df$y))` and `r round(max(main_df$y))`, it seems that just adding some basic, data-informed information to our priors resulted in more plausible results. This will generally help our models be more efficient and better behaved. Note that if all else fails, you can use a convenience function like [auto_prior]{.func style=""} demonstrated above.
...
The [tidybayes]{.pack style=""} package offers some nice options as well. It takes a bit of getting used to, but can be very handy. As an example, the following gives a visual sense of the probability of regression coefficient values beyond a chosen point of interest, in this case, arbitrarily chosen as `abs(.25)`.
...
library(patchwork)
{
  pp_check(model_baseline, ndraws = 50, re_formula = NA) + labs(title = 'No RE') + guides(color = 'none') +
    pp_check(model_baseline, ndraws = 50) + labs(title = 'With RE')
}  * lims(x = c(-3,10)) 
```
...
# library(patchwork)
# {pp_check(model_baseline, ndraws = 100, type ='stat', stat = 'min') +
#     pp_check(model_baseline, ndraws = 100, type ='stat', stat = 'max')}  /
#   {pp_check(model_baseline, ndraws = 100, type ='stat_grouped', stat = 'min', group = 'b2') + 
#       guides(fill = 'none') +
#     pp_check(model_baseline, ndraws = 100, type ='stat_grouped', stat = 'max', group = 'b2') + 
#       guides(fill = 'none')}
```
...
For more on Bayesian R^2^, see the [resources section](#r2)
...
In general, we'd always like to visualize the predictions. We can do so as we did before with the [conditional_effects]{.func style=""} function, which would also allow us to set specific covariate values. For the third plot of the nonlinear effect below, I modify the basic conditional effects plot that [brms]{.pack style=""} provides for a slightly cleaner visualization.
...
```{r conditional-effects-model-comparison-show, echo=F}
library(modelr)
...
With our new models added to the mix, we can now make some comparisons using [loo_compare]{.func style=""}. First, we'll add LOOIC estimates to our models, which are not estimated by default.
...
Let's look again at the basic result from using the [loo]{.func style=""} function.
...
Now what about those average predictions? Let's create a data frame that sets the continuous covariates at their means, and at each level of the categorical covariates. For our purposes here, we will also ignore group effects[^7]. We then will make average predictions for those observations using [pp_average]{.func style=""}.
...
[^8]: At the time of this writing, the underlying use of the [furrr]{.pack style=""} package defaults to not using a seed in it's parallelization process, and then warns you that a seed has not been set for each repeated use of a cluster. Passing a seed through the seed argument won't actually do anything presently here, so one will hope that [furrr]{.pack style=""} will change their default behavior. It's a nuisance that can be ignored though.
...
#| eval: false
save(
    model_interact_nonlin_cv,
    model_all_cv,
    file = 'data/bayes/practical-bayes-ii-model_cv.RData'
)
```
...
load('data/bayes/practical-bayes-ii-model_cv.RData')
```
...
save(
    var_select, 
    ref_model,
    file = 'data/bayes/practical-bayes-ii-var_select.RData'
)
```

```{r}
#| label: load-var_select
#| echo: false

load('data/bayes/practical-bayes-ii-var_select.RData')
```
...
```{r cv-feature-selection-show, echo=FALSE}
best = summary(var_select)$perf_sub |> 
    filter(elpd.diff > 0) |>
    slice(1) |> 
    pull(size)

summary(var_select)$perf_sub |> 
    rename(
        ranking = ranking_fulldata,
        cv_proportions = cv_proportions_diag
    ) |> 
    kable_df(digits = 1)
```
...
```{r}
#| label: cv-feature-selection-plot
...
-   If trying to select among many competing models, e.g. feature selection, you should consider why you are in this situation. If you don't have much data, then the usual model selection criteria may lead you notably astray. If you have a lot of data, consider why you need to select a subset of predictors and not use all available. If you are somewhere in between, note that you'll likely spend a lot more time here and still not be confident in the results. However, there are approaches, such as those in the [projpred]{.pack style=""} package, that might be useful, but likely will only work for simpler models.
...
### Model Averaging {#model-averaging-1}
