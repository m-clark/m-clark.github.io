...
description: Demonstrating some times series approaches
date: '2022-08-10'
image: ../../img/time-series/dalle_mini_time_series_seasonal_effect.png 
bibliography:
  - ../../bibs/ts.bib
draft: false
keywords: [time series, generalized additive models, mixed models, torch, deep learning, RNN, seq2seq, prophet, fable, lightgbm, gradient boosting, pytorch]
categories:
  - mixed models
  - GAM
  - boosting
...
execute: 
  ...
  cache: true
share:
    permalink: 'https://m-clark.github.io/posts/2022-08-10-time-series/'
    description: 'Exploring Time'
...
# Note that everything for this doc is now eval: false by default
...
slice = dplyr::slice # bc loading the models somehow loaded lightgbm::slice 
```
...
I start with data having already been processed, but as mentioned the source is publicly available. I use [data.table]{.pack style=""} to read it in more quickly, but it's default date class can cause issues with other packages, so I deal with that. I also extract the year, month, weekday, etc.
...
We split our data into training and validation sets, such that everything before `r SPLIT_DT` is used for training, while everything after will be used for testing model performance[^2].
...
save(
  ...
  red_line_train,
  red_line_validate,
  file = 'data/time-series/train-test-split.RData'
)
```

### Other

Holidays are available via the [prophet]{.pack style=""} package, which we'll be demonstrating a model with later. The data we're using already has a 'holiday vs. not' variable for simplicity, though it comes from a different source. The [prophet]{.pack style=""} version has both the actual date and the observed date counted as a holiday, and I prefer to use both.
...
save(
  holidays,
  file = 'data/time-series/holidays.RData'
)
```
...
We'll use [yardstick]{.pack style=""} to help us evaluate performance for this and subsequent models. In this case however, the visualization told us enough- a basic ARIMA isn't going cut it.
...
One nice thing about the [forecast]{.pack style=""} package is that it can include additional features via the `xreg` argument, which is exactly what we need- additional information. Now our model looks something like this, where $X$ is our model matrix of features and $\beta$ their corresponding regression weights.
...
In the following code, we create a model matrix similar to the training data that we can feed into the [predict]{.func style=""} function. The forecast package also offers a [glance]{.func style=""} method if desired.
...
## Mixed model with AR Structure {#mixed-model-with-ar-structure}
...
#| eval: false
#| echo: false
#| label: save-mixed-train-test-split
save(
  df_train_mixed,
  df_validate_mixed,
  file = 'data/time-series/mixed-train-test-split.RData'
)
```
...
[^4]: I always appreciated the depiction of this topic in @west2022 quite a bit.
...
As with [glmmTMB]{.pack style=""}, I use a custom function to summarize the model, or extract different components from it. From the initial glance we can see things that we expect (e.g. line and weekend effects are large).
...
The [prophet]{.pack style=""} package in R takes some getting used to. We have to have specific names for our variables, and unfortunately have to do extra work to incorporate categorical features. We can use [recipes]{.pack style=""} (like [yardstick]{.pack style=""}, part of the [tidymodels]{.pack style=""} 'verse) to set up the data (e.g. one-hot encoding).
...
We now visualize predictions as we did with the [GAM](#generalized-additive-models). But one of the nice things with [prophet]{.pack style=""} is that you can plot the various parts of the model results via the [plot ]{.func style=""} method or [prophet_plot_components]{.func style=""} (not shown). Unfortunately, our baseline effort seems to undersmooth our more popular lines (blue, red), and overreacts to some of the others (purple, yellow). This is because it's not really changing the predictions according to line.
...
We can also assess performance as before, but note that [prophet]{.pack style=""} has it's own cross-validation capabilities which would be better to utilize if this was your primary tool. Between the previous visualization and these metrics, our first stab doesn't appear to do as well as the GAM, so you might like to go back and tweak things.
...
I came across [fable.prophet]{.pack style=""} as a possibly easier way to engage prophet. It is an extension of [fable]{.pack style=""} and related packages, which are very useful for time series processing and analysis. Note that it is 0.1.0 version development, and hasn't had much done with it in the past year, so your mileage may vary with regard to utility by the time you read this[^5]. But with it we can specify the model in more of an R fashion, and we now don't have as much data pre-processing.

[^5]: A year plus later after that statement, it still hasn't gone beyond 0.1.0, so I don't think this will continue to be useful for very long. Unfortunate, but honestly, it's not clear <span class="pack" style = "">prophet\</span itself can do much better than many other tools.

### Data Prep

One key difference using [fable.prophet]{.pack style=""} is that it works with `tsibble` objects, and thus must have unique date observations. We can do this by setting `line` as the key[^6].

[^6]: [fable.prophet]{.pack style=""} may have a bug enabling the holidays functionality with parallel, so you can just use the original holiday column if you do so (single core doesn't take too long).
...
save(
  ...
  df_validate_fable,
  holidays_fable,
  file = 'data/time-series/fable-data-prep.RData'
)
```

### Model

Beyond this we use functions within our formula to set the various components. With line as the key, [fable]{.pack style=""} is actually running separate prophet models for each line, and we can do so in parallel if desired.
...
With [fable.prophet]{.pack style=""} visualization, we have the more automatic plots, but again we'll stick with the basic validation plot we've been doing.
...
This model does well, and better than basic [prophet]{.pack style=""}, but we can see its limitations, for example, with the yellow line, and more recent ridership in general.
...
p_dat2 = layer_data(p, 2)  # main value
...
And we check performance as before. The [fable]{.pack style=""} model is doing almost as well as our [GAM](#generalized-additive-models) approach did.
...
# upon revisiting, this would not render even though there is no error with the actual code, so the table is saved instead

# fable_accuracy = accuracy(forecast_fable, df_validate_fable) %>% 
...
The [fable]{.pack style=""} results suggests what we already knew from our [GAM](#generalized-additive-models) and [mixed model](#mixed-model-with-ar-structure) approach, that interactions of the series with line are important. We weren't easily able to do this with the default [prophet]{.pack style=""} (it would likely require adding time x line interaction regresssors), so it's nice that we have the option here.
...
I'll use [lightgbm]{.pack style=""} which I've been increasingly using of late. It requires a matrix object for input, and so some additional processing as well.
...
Typically we would perform some sort of search over the (many) parameters available to tweak with [lightgbm]{.pack}, like the number of trees, learning rate, regularizer parameters and more. I ignore that, but I did fiddle with the learning rate and bumped up the `nrounds` (trees), but that's it.
...
# Originally used cv but this is unnecessary.
# fit_gbm = lgb.cv(
#   params    = params
#   , data    = df_train_lgb
#   , nrounds = 2000L
#   , nfold   = 5L
# )
lgb.save(fit_gbm, 'data/time-series/gbm.txt')
...
fit_gbm = lightgbm::lgb.load('data/time-series/gbm.txt')

p_gbm = df_validate %>% 
  droplevels() %>% 
  mutate(pred = fit_gbm$predict(X_test)) %>%
  mutate(date = as_date(date)) %>% 
  ggplot(aes(date, rides_scaled)) +
  geom_line(alpha = .1) +
  geom_line(aes(y = pred, color = I(colors)), alpha = .25) +
...
p_gbm_red = df_validate %>% 
  droplevels() %>% 
  filter(line == 'red', year == 2018) %>% 
  mutate(pred = fit_gbm$predict(X_test[df_validate$year == 2018 & df_validate$line == 'red',])) %>%
  mutate(date = as_date(date)) %>% 
  ggplot(aes(date, rides_scaled)) +
  geom_line(alpha = .1) +
  geom_line(aes(y = pred, color = I(colors)), alpha = .25) +
...
save(
  ...
  train_mean,
  train_sd,
  file = 'data/time-series/torch-data-prep.RData'
)
```
...
# load all the objects This is now done at the beginning of the document
# x = list.files('data/time-series/', pattern = 'RData', full.names = TRUE)
# purrr::map(x, load, envir = .GlobalEnv)
```

```{r vis-all-lines, echo=FALSE, out.height='125%'}
library(patchwork)
