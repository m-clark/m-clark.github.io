...
Understanding when different models excel requires looking at how dataset characteristics relate to performance.
...
- Binary: DL typically edges out boosting
- Multiclass: DL (especially TabPFN-v2.5) shows consistent advantages
...
- **Very large data (>1M rows)**: Boosting is your primary choice. Few DL models scale or require prohibitive resources. But keep an eye on this, particularly with the recent success of TabICL-v2 on larger datasets.
...
One thing to also keep in mind, even a linear model still beats even DL models 5-10% of the time, so you just never know.
...
Foundation models represent an interesting future direction but currently remain limited by data size constraints and computational costs. The rapid progress suggests checking back in another year might reveal more practical options. For now, start with boosting, experiment with DL if resources permit, and let your specific constraints guide the decision rather than expecting a universal answer.
