...
description: Challenges and solutions for this common situation
date: 2025-04-07
image: ../../img/class-imbalance/roc_curve.png
tags: ['classification', 'imbalanced data', 'thresholds', 'roc', 'pr', 'cost-sensitive learning', 'resampling', 'ensemble methods', 'smote']
author: 
    - name: Michael Clark
      email: stats.data.sci@gmail.com
...
- A lower threshold, which would lead to more positive classifications, may be appropriate when missing positive cases (false negatives) is costly, such as in disease detection[^eitherway].
...
### The Confusion Matrix {#sec-confusion-matrix}
...
### Example: Classification with a .5 Threshold {#sec-example-thresh-05}
...
## Case Study: Wine Quality Data {#sec-case-study}
...
df_metrics = df_metrics.assign(**{'Threshold Method': np.where(df_metrics['Threshold Method'].isna(), 'Default', 'F2')})
...
We use two base models, a standard logistic regression model via [sklearn]{.pack} and a boosting model via [LightGBM]{.pack}. Both use all non-target columns as features. We will compare the performance of the following models:
...
- A weighted model (using `class_weight='balanced'` for the logistic and `is_unbalance=True` for the LGBM)[^wts]
- A random oversampling approach (oversampling) [^nosmote]
...
# since thresh is dropped, min/max indices are different
(
    GT(df_metrics_logistic[prob_based].query('Model != "Logistic Basic (T)"'))
...
# decided to drop thresh so min/max metrics need to be altered
(
    GT(df_metrics_lgbm[prob_based].query('Model != "LGBM Basic (T)"'))
...
df_combined.rename(columns={'Threshold Method': 'Optimization Method'}, inplace=True)
