...
date:  2019-08-05 # '`r format(Sys.Date(), "%B %d, %Y")`'
preview: ../../img/compare_group_latent.png   
output:
...
<aside>Recall that for the standard latent linear model, the observed variable is the dependent variable . For example, given an observed variable $X$, a latent variable $F$ and loading $\lambda$: $$X = b_0 + \lambda F $$</aside>
...
This last one just compares the means.  We see that the 
