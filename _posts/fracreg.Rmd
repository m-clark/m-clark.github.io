...
It might seem strange to start with an example using Stata.  But if you look this sort of thing up, you'll almost certainly come across the Stata example using the <span class="func" style = "">fracreg</span> command.  The following specifies a fractional regression with logit link. It is based on the example used in the [documentation](https://www.stata.com/manuals/rfracreg.pdf) for the command.
...
We can get robust standard errors for the quasilikelihood approach as well, but they were already pretty close.
...
It turns out that we can also used a [mixed model](https://m-clark.github.io/mixed-models-with-R/) approach.  For some distributions, such as binomial and poisson where the variance is directly tied to the mean function and does not have to be estimated, we can insert a per-observation random effect.
