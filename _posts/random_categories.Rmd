...
# the order in the plot doesn't match the ordered worker factor 
machines %>% 
...
The random effects of interest are for worker and machine, so how do we specify this?  Let's try the following standard approach.

```{r model_m_slope}
model_m_slope = lmer(score ~ Machine + (1 + Machine | Worker), machines)
```

Exactly the same issue our client had.  Problematic convergence.  This could more of an issue with `lme4`, and we could certainly explore tweaks to make this issue go away (or use a different package like `glmmTMB`), but let's run with it.

```{r m-slope-results}
summarize_model(model_m_slope, ci = FALSE, cor_re = TRUE)
```

We get the variance components we expect, The variance attributable to the intercept (i.e. Machine A), as well as for the slopes for the difference in machine B vs. A, and C vs. A.  We also see the correlations among the random effects.  It's this part that Bates notes is hard to estimate and incurs estimating potentially notably more parameters than typical random effects models.  In this case we have different options that will be available to us.  
...
Examine the random effects to make clear the difference.  For our first baseline model, we only have 6 effects, one for each worker
...
Now let's add an interaction of worker with machine rather than machine as a random slope.  This combines our two baseline models
...
Next we do the 'vector-valued' model Bates describes.
...
Now let's extract the fixed effect and variance component summaries.
