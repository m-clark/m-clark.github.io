...
bibliography:
  ...
  - ../../bibs/ts.bib
draft: true
tags: [time series, mixed models, torch, deep learning, RNN, seq2seq, prophet, pytorch]
categories:
...
In what follows I will show some more detailed code regarding the output, e.g. how I obtain the basic plot shown, but won't show it later for conciseness, focusing mostly just on the basic model code. You can always find the code for these posts on my [GitHub](https:://github.com/m-clark/m-clark.github.io).


## Data Description

As mentioned on Cody's post, over 750,000 riders use the Chicago Transit Authority's 'L' system to get around the city. There are 8 interconnected rail lines (named after colors- the Red, Blue, Green, Brown, Pink, Orange, Purple, and Yellow lines), 145 entry/exit stations, and over 2,300 combined trips by its railcars every day[^express].
...
In Cody's post, he added pertinent information regarding weather, sporting events, and more.  I have saved the processed data here.

`TODO: NEED LINK`   

So we have daily ridership from 2012-2018, and want to model this in some fashion.  We will use a normalized ride count (mean zero, standard deviation 1) as our primary focus.
...
I start with data having already been processed, but as mentioned, the original data can be found [here](), and the source is publicly available.
...
A quick look at the red line similar to Cody's post.
...
red_line_train %>%
  ...
  geom_vline(
    aes(xintercept = date),
    data  = . %>% filter(date == floor_date(date, unit = 'year')) %>% distinct(),
    lty   = 'dashed',
    size  = .5,
    alpha = .5
  ) +
  scale_x_date(
    breaks = seq.Date(as_date('2012-01-01'), as_date('2017-07-01'), by = '6 months'),
    date_labels = '%Y-%m'
  ) +
  scale_y_log10(breaks = seq(0, 3e5, 5e4)) +
  labs(x = '') +
...
mm = model.matrix(
  ~ . - 1, 
  data = red_line_train %>% 
    select(is_weekend:is_sox_game, tmax_scaled, prcp_scaled)
)
...
model_mixed = 
  ...
  (1|year) +              # doing year as factor will cause issues since 2018 isn't in train
  (1|day) + 
  (1|day:line) + 
  (1 + year_day|line)
...
```{r mixed-ar-explore, message=TRUE}
library(mixedup)
