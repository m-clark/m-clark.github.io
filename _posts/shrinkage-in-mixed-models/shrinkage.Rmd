...
Now it is time to visualize the results.  We will use <span class="pack">gganimate</span> to bring the shrinkage into focus.  We start with the estimates that would be obtained by a fixed effects, or 'regression-by-cluster' approach.  The movement shown will be of those cluster-specific estimates toward the mixed model estimates.  On the x axis is the estimate for the intercept random effects, on the y axis are the estimated random effects for the slope of the `x` covariate.  The plots for this and others are zoomed in based on the random effect variance inputs for this first model, which attempts to make the plots more consistent and comparable across the different settings.  As such, the most extreme points often will not be shown at their starting point.
...
If we have fewer observations within each cluster, the more likely extreme values will present in the by-cluster approach, and thus more shrinkage is applied using a mixed model. We have relatively less knowledge about the groups, so we would prefer to lean toward the population average. In the most extreme case of only one observation per cluster, we simply have the standard regression model and its associated fixed effects.
...
[^hownormalworks]: Think of a normal distribution with standard deviation of 1- almost all of the likely fall between 3 and -3. The probability of values beyond that would be less than .001.  If the standard deviation is 3, the probability of a value beyond $\pm$ 3 is ~ .32, i.e. very likely.
