...
`X` is a standardized variable with mean zero and standard deviation of 1. Unless a fraction is provided for `balanced`, the N is equal to `n_cluster` * `obs_per_cluster`. The following is the function that will be used to create the data, which follows the model depiction above.
...
Now it is time to visualize the results.  We will use <span class="pack">gganimate</span> to bring the shrinkage into focus.  We start with estimates as would be obtained by a fixed effects, or regression-by-cluster approach.  The movement of the points is toward the mixed model results for the same cluster-specific effects.

```{r vis_baseline_model, echo=FALSE, fig.asp=4/3, eval=F}
plot_data = create_plot_data(model = mod)
p = plot_fun(plot_data = plot_data, end_pause = 20)
print(p)
anim_save('img/shrinkage/baseline.gif', options = list(fig.asp=4/3))
```

<!-- Relative to post folder -->
<img src="../../img/shrinkage/baseline.gif" style="display:block; margin: 0 auto;">

We see more clearly what the mixed model does.  The general result  is that cluster-specific effects are shrunk back toward the population-average effects, as the imposed normal distribution for the random effects makes extreme values less likely. In terms of prediction, it is akin to introducing bias while lowering variance for prediction of new data, and allows us to make predictions on new categories we have not previously seen - we just assume an 'average' cluster effect, i.e. a random effect of 0.
...
If we add more slope variance relative to the intercept variance, this more  or less changes the orientation of the plot.  
...
If we have fewer observations within each cluster, more shrinkage is applied. We have relatively less knowledge about the groups, so prefer to lean toward the population average.
...
The opposite case is seen with more observations.  We also see that the estimates do not so easily fly to extreme values to begin with.  With enough observations per cluster, you likely will see little to no shrinkage.
...
With unbalanced data, we see the combination of having more vs. fewer observations per group. Those clusters with more observations will generally exhibit less shrinkage, and those with fewer observations the opposite.  

```{r unbalanced, echo = 1}
df = create_data(balanced = .5, seed = 12)
mod = lmer(y ~ x + (x|cluster), df)
plot_data = create_plot_data(model = mod)
...
cor(plot_data$coefficients %>% filter(model == 'mixed') %>% select(Intercept, x)-
    plot_data$coefficients %>% filter(model == 'lm') %>% select(Intercept, x),
    plot_data$coefficients %>% filter(model == 'lm') %>% select(n_clus))
