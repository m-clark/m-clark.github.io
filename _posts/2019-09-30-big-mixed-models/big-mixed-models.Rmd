...
tags: [random effects, mixed models, lme4, glmmTMB, mgcv, big data]
---
...
The following are some guidelines for when bam might be preferable compared to other mixed modeling tools.

### Linear Mixed Models

In general you'll need very large data for bam to be preferred to lme4 for linear mixed models unless:

- You have complicated structure that begins to bog down lme4
- You want to add smooth terms[^smoothlme4]
- You have memory issues

The following shows some timings for lme4, glmmTMB, and mgcv for the linear mixed model case under a variety of settings.  In some sense, this is not exactly a fair comparison as mgcv parallelizes computations while lme4 and glmmTMB do not.  However, this is also exactly the point of the demonstration - those who can, do.  In general though, the lme4 advantage holds until around 1 million observations. We can see that the main issue for bam is not so much the sample size, but the number of parameters to estimate.

<aside>For lme4, I set at least one setting to possibly improve speed performance for both lme and glmm models, though this only shaved a few seconds for the largest sample size settings for the linear mixed model.  For mgcv I only used 10 cores for parallelization so as to be similar to what is common on modern machines (8-12), but anyone with access to a cluster computing environment would see even more speed gain by utilizing additional cores. For glmmTMB, settings were left at defaults, as I've not come across any specific recommendations. See @brooks_glmmtmb_2017 for more speed comparisons of glmmTMB, mgcv, lme4, and others, as well as the [glmmTMB vignette](https://cran.r-project.org/web/packages/glmmTMB/vignettes/glmmTMB.pdf).</aside>
...
- glmmTMB is probably viable up to 100k and one or two random effects
- Use mgcv for same reasons as above

Beyond that, I think bam will be a great choice.

  
### Limitations

To summarize the limitations of the mgcv approach, we can note the following:
...
[^smoothlme4]: You could use construct the smooth with mgcv and add it to the model matrix for lme4.
...
[^mccoach]: See McCoach et al. (2018) [Does the Package Matter?](https://journals.sagepub.com/doi/abs/10.3102/1076998618776348).  They also look at HLM and Mplus. However, I haven't in years consulted with anyone across dozens of disciplines that was using it for mixed models. With Mplus, the verbosity of the syntax, plus additional data processing required, plus huge lack of post-processing of the model would negate any speed gain one might get from simply running the model. Couple this with the fact that campus-wide licenses are rare for either, neither could be recommended for mixed models.  Note also, that one setting of lmer probably would have negated almost all their convergence issues.