...
From Statistical Learning from a Regreession Perspective 3e, fig 1.9
...
calc_cp_error <- function(
  ...
  test_resid = abs(predict(fit, newdata = test) - y)
  
  k = ceiling((nrow(test) + 1) * interval)
...
cp_error = calc_cp_error(fit, test, y = test$ln_house_value)
