...
date: 2019-08-20 # '`r format(Sys.Date(), "%B %d, %Y")`'
...
- Beta regression for (0, 1), i.e. only values *between* 0 and 1 (see [betareg](https://cran.r-project.org/web/packages/betareg/vignettes/betareg.pdf), [mgcv](https://cran.r-project.org/web/packages/mgcv/), [brms](https://cran.r-project.org/web/packages/brms/)  packages)
...
So now we have the same result via a standard R generalized linear model and Stata.  Likewise, you could just use the glm command in Stata.
...
<aside>From the R help file for `?family`: The quasibinomial and quasipoisson families differ from the binomial and poisson families only in that the dispersion parameter is not fixed at one, so they can model over-dispersion.  

Also, as noted in the StackExchange link in the [references][references], while by default the variance estimate is 'robust', possibly leading to standard errors that are similar, the basic result is not the same as using the robust standard errors.</aside>
...
In addition, we can see what the <span class="pack" style = "">frm</span> package, which is specifically for fractional regression, would produce.  Unless you really need this tool for the more complicated scenarios it has some functionality for (e.g. panel data), this isn't as user friendly an approach as the others[^frm]. Like Stata's specialized command, it is equivalent to using the quasibinomial family with robust standard errors.
...
StackExchange has some more useful discussion, e.g. more on standard error differences between the approaches [link](https://stats.stackexchange.com/questions/89999/how-to-replicate-statas-robust-binomial-glm-for-proportion-data-in-r/205040)
...
[^frm]: As mentioned, the [frm package](https://cran.r-project.org/package=frm) may not be user friendly enough for many.  It doesn't use data frames, requires inputs that separate variables from the data matrix, lacks typical model methods (e.g. <span class="func" style = "">coef</span>, <span class="func" style = "">predict</span>), and I'm not sure it's still being actively developed, among other things.
