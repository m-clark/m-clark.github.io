...
preview: <i class="fab fa-percent fa-5x"></i>   # apparently no way to change the size displayed via css (ignored) or file (stretched)
output:
...
Some suggest adding a fudge factor to the zeros or ones to put them on the (0, 1) interval. However, such hacks are unnecessary these days.  Furthermore, you already have tools that model this sort of target variable, and this post will demonstrate some of them.


## Stata example

It might seem strange to start with an example using Stata.  But if you look this sort of thing up, you'll almost certainly come across the [Stata example](https://www.stata.com/features/overview/fractional-outcome-models/) using the <span class="func" style = "">fracreg</span> command.  For comparison we'll use the data in the corresponding [documentation](https://www.stata.com/manuals/rfracreg.pdf).  The data regards the expected participation rate in 401(k) plans for a cross-section of firms. They define participation rate (`prate`) as the fraction of eligible employees in a firm that participate in a 401(k) plan.  This is modeled by the matching rate of employee 401(k) contributions (`mrate`), the natural log of the total number of employees (`ltotemp`), the age of the plan (`age`), and whether the 401(k) plan is the only retirement plan offered by the employer (`sole`).  Here we do not use quadratic effects, though we do use an additive modeling approach later that could be used instead.
...
This is one of the few tools that is readily available to model such outcomes, but as we're about to see, you definitely don't need Stata once you know what's going on.
...
As such, we can just use <span class="func" style = "">glm</span> like we would for count/binary outcomes. It will warn you that the outcome isn't integer as it expects, but in this case we can just ignore the warning.
...
So the model runs fine, and the coefficients are the same as the Stata example.  The standard errors are not, but we can fix that.


## Robust standard errors

By default, Stata does robust standard errors. We can use the <span class="pack" style = "">sandwich</span> package to get them in R.  The <span class="pack" style = "">lmtest</span> package provides a nice summary table.
...
So now we have the same result via base R and Stata.
...
It turns out that we can also use a [mixed model](https://m-clark.github.io/mixed-models-with-R/) approach.  For some distributions, such as binomial and poisson where the variance is directly tied to the mean function and does not have to be estimated, we can insert a per-observation random effect.  This extra source of variance can account for overdispersion, similar to what the scale parameter estimate does for the quasibinomial.  I attempted to do so using <span class="pack" style = "">lme4's</span> <span class="func" style = "">glmer</span> with the observation level RE.  While I've had success with such models in the past, all failed to converge for this example with default optimization settings across multiple optimizers. As such, the results are not shown.
...
[^frm]: There is a [frm package](https://cran.r-project.org/package=frm) for fractional regression, but I don't recommended it.  It doesn't use data frames, requires inputs that separate variables from the data matrix, and doesn't appear to be actively developed, among other things.