...
<!-- <span class="pack" style = "">frm</span> package demo. Compare with the results above, specifically, the quasibinomial with robust standard errors.  This  -->
...
<!--   x_mat, -->
<!--   linkfrac = 'logit', -->
<!--   intercept = FALSE  # included in model matrx -->
<!-- ) -->
<!-- ``` -->
...
[^frm]: There is a [frm package](https://cran.r-project.org/package=frm) for fractional regression, but it may not be user friendly enough for many.  It doesn't use data frames, requires inputs that separate variables from the data matrix, lacks typical model methods, and I'm not sure it's still being actively developed, among other things.
