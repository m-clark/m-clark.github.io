...
world_death_rates = world_totals %>%
  ...
  summarise(
    ...
    `Death Rate (minus Italy & Spain)` = 
      sum(total_deaths[!country_name %in% c('Spain', 'Italy')]) / 
      sum(total_confirmed[!country_name %in% c('Spain', 'Italy')])
  )
...
washtenaw_cases %>% 
  ...
  select(-`Total Hospitalizations`, -TotalRecovered) %>% 
...
washtenaw_plot =  county_trends %>% 
  ...
  geom_path(aes(), alpha = .5, color = '#ff5500', size = 1) +
  geom_point(
    aes(color = 100 * (confirmed/lag(confirmed) - 1)),
    size  = 5,
    alpha = .5
  ) +
  geom_point(
  ...
  theme(
    axis.ticks.y = element_blank(),
    panel.grid.major.y = element_line(color = '#ff550040', size = .1),
    legend.key.size = unit(.25, 'cm'),
    legend.text     =  element_text(size = 6),
    legend.title    = element_text(size = 6)
  )
...
I've started playing with some [hospital data for a dashboard](https://m-clark.github.io/docs/covid_staffing/hospital_icu_covid.html).  The (stilly pretty messy) source code is available on a link there as well, so feel free to play with it for your own needs.

## Models

The following models U.S. state daily trends. First we'll download data from NY Times and add a bit of mild cleanup.  It seems to be cleaner than some other sources.

### US State Trends

```{r us-state-daily-model-data}
...
us_for_model = us_states0 %>% 
  ...
  mutate(state_abb = as.factor(state_abb))  # for mgcv

# us_for_model %>% filter(state == 'Michigan')
```

To explore the trends, I will use a generalized additive model, with a separate smooth for each state.  It's not clear to me whether a specific count distribution is really required given the very high number of daily counts, but we'll go ahead and use the negative binomial.  I also use a specific function to speed up processing, but this is not required for those willing to wait.

```{r us-state-daily-model, cache=TRUE}
library(mgcv)

model_state = bam(
  daily_cases ~ s(date_num, state_abb, bs = 'fs', k = 15),
  family   = nb,
  data     = us_for_model,
  nthreads = 10
)
...
The following plots the derivatives of each state curve. Each dot reflects a state's peak positive rate of change.  While most peaks are past, the expected trend is still positive for most states.

```{r us-state-model-deriv, echo = F, cache=TRUE}
library(gratia)
...
```{r us-state-model-deriv-plot, echo = F}
library(ggrepel)
...
The following predicts the next ten days for the ten states with the most confirmed cases.

```{r us-state-model-predict, echo = F}
start = as.numeric(Sys.Date())
end = start + 10
...
```{r us-state-model-predict-plot, echo=F}
...
pred_data %>%
  ...
  labs(x = '', y = '') +
  scico::scale_color_scico_d(begin = .1, end = .9) +
  scale_y_continuous(breaks = seq(0, max_pred, by = 2500), trans = 'log') +
  visibly::theme_clean() +
  theme(
    axis.ticks.y = element_blank(),
    legend.position = 'bottom',
    panel.grid.major.y = element_line(color = scales::alpha('black', .025))
  )
```
