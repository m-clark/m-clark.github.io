...
This is just some code to get processed data and visualizations regarding the COVID-19 outbreak.  It demonstrates some data pulls from the Wikipedia tables of WHO incidence reports, as well as the data used to produce the Johns Hopkins Dashboard.  The goal here is just some stuff to make it easier for others to get their hands dirty.


I actually started scraping Wikipedia's WHO incidence report tables, but any cursory glance showed numerous issues and lots of cleaning, coupled with a format that changed almost daily.  I then started playing with the data behind the [Johns Hopkins dashboard](https://coronavirus.jhu.edu/map.html), which was better, but didn't have some info and there are issues there as well. I settled on Open Covid, but may revisit JH after their changes take place, as they will also have U.S. county level counts.
...
The following is a pretty simple function for reading data from the GitHub repo. It has arguments for filtering to a specific country, whether or not you only wan the current numbers, 
...
read_covid_data <- function(
  ...
  data = data %>% 
    ...
    select(date:region_name, contains('daily'), contains('total'), everything()) %>% 
    ungroup()
...
```{r table-of-world-current-counts, echo=FALSE}
world_death_rates %>% 
...
```{r table-of-us-current-counts, echo=FALSE}
us_death_rates %>% 
...
The following is adapted from the ReadMe file.
...
| **daily_confirmed** | Daily number of cases confirmed after positive test | 67800 |
| **daily_deaths** | Daily number of deaths from a positive COVID-19 case | 3139 |
...
The following does some additional processing before going to a plot of the trends.
...
world = countries %>%
  filter(date != Sys.Date() + 1) %>%   # current days data may not be complete
  group_by(date) %>% 
...
```{r country-trend-static-save}
ggsave('img/covid.svg')
```


The following animates the previous.

```{r country-trend-animate, dev='png', eval=T, echo = -1, dev.args=list(bg = '#fffff8')}
...
```{r country-trend-animate-show, eval=FALSE}
p_animate
```


```{r country-trend-animate-save, echo=FALSE, eval=-1, cache=FALSE}
...
The following is similar to the plot shown on the [New York Times daily count](https://www.nytimes.com/interactive/2020/world/coronavirus-maps.html), just without the unnecessary discretizing of the color (the legend already does that for you).  I chose a similar palette, but obviously you can play around with that.
...
```{r state_trends, cache.rebuild=F}
...
p = us %>% 
  ...
  visibly::theme_clean() + 
  labs(x = '', caption = 'Dark large dot is US total') +
  theme(
    axis.text.x  = element_text(size = 6),
    axis.ticks.y = element_blank(),
    legend.title       = element_blank(),
...
Here we look at counts and death rates via a 'binned' map.
...
We can plot trends in a map-like fashion too.
