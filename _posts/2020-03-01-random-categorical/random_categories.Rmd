...
Next we'll do the 'vector-valued' model Bates describes.  This removes the intercept portion of the formula in the random slopes model, but is otherwise the same. We can look at the results here, but I will hold off description for comparing it to other models. Note that at least have no convergence problem.
...
At this point we've seen a couple of ways of doing a model in this situation.  Some may be a little too simplistic, others may not capture the correlation structure the way we'd want. 
...
The following is a simple approach to creating data in this scenario, and allows us to play around with the settings to see what happens.  
...
Now I will make the `vector_valued` model reduce to the `separate_re` model.  First, we create a covariance matrix that has equal variances/covariances (i.e. compound symmetry), and for demonstration, we will apply the random effects a little differently.  Then, when we create the target variable, we make a slight alteration to apply it to the vector valued model instead.
...
cov2cor(cov_mat)  # this will now be the estimated correlations for the vector_valued model
