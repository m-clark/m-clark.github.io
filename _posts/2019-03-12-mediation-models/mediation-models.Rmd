...
<aside> The result is based on simulations of a multivariate normal draw of the coefficients given their estimated covariance matrix. The algorithm is summarized as follows (from Imai et al. 2010)
...
With this approach we can obtain the average difference and corresponding quantiles based on the simulated draws.</aside>
...
<aside>The plot is using <span class="pack" style = "">Diagrammer</span> and <span class="pack" style = "">graphviz</span> under the hood, which is appealing to me as I use those anyway, so could use the generated code as a starting point.</aside>
...
<aside>There used to be a package to calculate indirect effects of a <span class="pack" style = "">psem</span>  object, semEff, but it currently [doesn't work](https://github.com/murphymv/semEff/issues/5).</aside>
...
```{r bnlearn_reproduction, eval=T, fig.height=5, cache.rebuild=F}
library(bnlearn)
...
# reticulate is a clusterfuck at present; documentation is next to useless,
# numerous open issues that may or may not be relevant to one of the many error
# messages you may get depending on how you breath on the settings, sheer luck
# got it to work

# library(reticulate)
# knitr::opts_chunk$set(python.reticulate=FALSE)
# reticulate::use_python("python/bin/python")

# jobs = data(jobs, package = 'mediation')
# Sys.setenv(RETICULATE_PYTHON = "~/anaconda3/bin/python3")
# reticulate::use_condaenv()
```

```{python pymediate, python.engine = "Users/micl/anaconda3/bin/python3", echo=-6, cache=T}
