...
tibble(
  ...
  kable_styling(full_width = F) %>% 
  footnote(symbol = c('approximately', "Standard linear model, as estimated by `lm`"),
           symbol_manual = c('*', '<sup>&dagger;</sup>'))
...
[^mass]: Sorry, but <span class="pack">MASS</span> has been superseded by others for over a decade at this point. Now all it does is muck up your tidyverse when it's loaded.  It's a fine package (and was great back in the day), but if you want to use it, don't load it in the environment.  All I see it used for these days is <span class="func">mvrnorm</span> (multivariate normal distribution) and <span class="func">glm.nb</span>, but I would use others for those reasons that would provide additional benefits and not mask <span class="pack">dplyr</span> functions. 
