...
nocite:  | 
tags: [stata, margins, prediction, average marginal effects, marginal effects at means, estimated marginal means, offset, emmeans, ggeffects, regression, glm]
categories:
  - regression
  - GLM
---
...
If we add the offset to the `spec` argument, it still just fixes it at the mean.  So at least using the standard approaches with this model does not appear to give you the same thing as Stata.
...
```{stata margins-fix-offset, engine.path=stataexe, echo=-(1:2), consolelog=F}
use "data/insurance.dta"
qui nbreg claims i.age, offset(ln_holders) nolog
margins age, atmeans
...
The <span class="pack" style = "">margins</span> package explicitly attempts to duplicate Stata's margins, but here we can see it has an issue with the offset.
...
The offset function is part of the stats package of the base R installation, so I tried rerunning the model using `stats::offset`, but this did not lead to a result I could duplicate in an obvious way.  However, in the end I think it is something about the way the <span class="objclass" style = "">negbin</span> class is not handled (which isn't explicitly supported by <span class="pack" style = "">margins</span>, though it is also <span class="objclass" style = "">glm</span> class, which *is* supported).  Changing the model to poisson actually produces the results for the differences in predictions for each group from the reference group.  We'll come back to this type of result later.

```{r margins-poisson}
pois_glm_offset = glm(
  claims ~  age + stats::offset(ln_holders),
  data = insurance,
  family = 'poisson'
)
...
The over approach for the <span class="pack" style = "">margins</span> package. The package author states:
...
It would look something like the following, but we still have the offset warning, so I don't show the result.
...
Sometimes people want differences as you move from one level to the next for some covariate, the 'average marginal effect'.  In Stata this is obtained with the `dydx` option.
...
We can however use the <span class="pack" style = "">margins</span> package for this, and it provides the same result as before.  For whatever reason, it doesn't have an issue with the offset now.
