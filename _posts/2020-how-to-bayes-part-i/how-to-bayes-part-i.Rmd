...
We do not need as much data for our purposes so we'll set the total sample size to 1000.  We'll drop unnecessary columns, and also make our binary covariates explicit factors, which will make things easier when we want to visualize grouped effects later.
...
To summarize, the general solution to Rhat and ESS warnings is simply to do more iterations (relative to the warmup). To keep posterior samples and model objects from becoming unwieldy in size[^bigobjects], consider thinning also. Thinning will also reduce autocorrelation, as the draws retained after thinning are not as correlated with one another as successive draws would be.
...
- Scale the data: for any model, Bayesian or otherwise, scaling the data will result in a more manageable estimation process.
