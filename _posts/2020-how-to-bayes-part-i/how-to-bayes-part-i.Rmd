...
For applied analysts, I would suggest first focusing on the density plots and trace plots for parameters.  For the density plots of regression coefficients, these should be roughly normal looking, for variance parameters you may see skewness, especially if the estimate is relatively near zero with smaller data sets. Trace plots in general should look 'grassy' or like a 'fuzzy caterpillar', which isn't very helpful, but deviations are usually striking and obvious in my experience.  If you see chains that look like they are getting stuck around certain estimates, or separating from one another, this would indicate a serious problem.  If the chains are not converging with one another, you probably already were getting warnings and messages.  You can then examine other visualizations that might be appropriate to the problem, and take the appropriate steps outlined to try and solve those issues.  Pay extra attention to parameters with relatively low effective sample sizes, as these are the ones the model is struggling more so to estimate.
...
### Unhelpful solutions you can ignore

'Reparameterize the model/Use a change of variables.':  If it is suggested without a specifc implementation, it's almost useless advice.

'Your model is probably  misspecified.': So it is, so is any I come up with. Without suggestions toward a better specification, this doesn't help.

'Look for patterns in poor plots.': If it's not a good visualization to begin with, or 'patterns' isn't clearly defined, this isn't helpful.

'Read a highly technical article/several pages of technical documentation.': Few want to do this just to fix what should be a relatively minor issue. Many users will not have the context necessary for interpretation.


### Solutions to pay attention to

Those that offer very specific advice that has an obvious implementation in your current modeling approach.

Those that offer code that follows said advice.

It seems unfair to ask people for a reproducible example just to give unreproducible advice.
...
[^adapt_delta]: In my experience, there isn't a need to guess between .80 and .99 as the time differences are typically negligble, unless your model already takes a very long time.  Also, if it doesn't work at .99, it won't at .9999 either.
