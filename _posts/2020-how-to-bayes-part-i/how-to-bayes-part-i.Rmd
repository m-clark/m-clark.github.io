...
Here are some discussions on the Stan forums for getting through installation problems.  Unfortunately a couple of these threads are now nearly a year old, and weren't closed, so some are still posting to them.  This means that anything prior to R 4.0 can be ignored, and you probably only want to skip towards the end of those discussions for the relevant parts.
...
##### Examine effectiveness visually  

- <span class="func" style = "">pp_check</span>, tidybayes, etc.

##### Compare models using loo, posterior probabilities of models

- Avoid using [bayes factors](https://statmodeling.stat.columbia.edu/2019/09/10/i-hate-bayes-factors-when-theyre-used-for-null-hypothesis-significance-testing/), at least as a way to substitute the null hypothesis test you would have done in the non-bayesian setting, and for other technical reasons.
...
**What we want**: ESS > 10% percent of total samples for sure, but > 50% is best.
...
**What we want**: Good mixing of lines.
...
model_start = update(model_start_100, warmup = 1750, iter = 2000) # 1000 posterior draws
summary(model_start)
...
The help tells you the value should be somewhere between 0 and whatever `max_treedepth` is set at.  If they are large, the documentation states it could be due to three different things, all of which suggest either reparameterization of the model (probably not possible unless using Stan directly), or just increasing the value.  None of the documentation seems to tell you what those plots are supposed to look like, and unfortunately that renders them as not very useful. The divergence and energy plots are similarly underexplained.  Many refer users to Betancourt's wonderful articles on the details, but these are far too technical for those not already steeped in the approach, and even then[^betanwhat].
