...
We'll start with the estimated factor scores. In our model, we'll control for the verbal and speed constructs also.
...
Now lets do a sum score. It may not be obvious, but a sum score can be seen as assuming a latent variable model where
```{r sum_score}
lm_sum = lm(rowSums(HolzingerSwineford1939 %>% select(x1:x3)) ~ HolzingerSwineford1939$school)
